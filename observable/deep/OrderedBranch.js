...
export default compose(_Evented, _Destroyable, function(source, key, orderingSubKey, compareFn) {
    var self = this;
    this._source = source;
    this._key = key;
    ...
    this._own(source.onChange(function(change) {
        if (firstPathSegment(change.key) === key) {
            ...
            // si le changement correspond à la clé qui sert à ordonner, on regarde si ça change l'ordre
